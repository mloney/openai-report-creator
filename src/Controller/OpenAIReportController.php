...
use Drupal\custom_webform_handler\Service\ViewDataService;
...
class OpenAIReportController extends ControllerBase {
  ...
  public function __construct(MailManagerInterface $mail_manager, LoggerChannelFactoryInterface $logger_factory, Client $http_client, ViewDataService $view_data_service) {
  ...
  public static function create(ContainerInterface $container) {
    return new static(
      ...
      $container->get('custom_webform_handler.view_data_service')
    );
  }

public function testViewsInteraction() {
  return [
    '#type' => 'markup',
    '#markup' => $this->t('Test Views Interaction page'),
  ];
}
...
public function generateReport($values, $api_key, $assistant_id, $prompt, $view_name, $view_display_name) {
    ...
    if ($this->viewDataService) {
        $view_html = $this->viewDataService->getViewData($view_name, $view_display_name);
        if (!empty($view_html)) {
            $this->logger->notice('View HTML successfully extracted.');
        } else {
            $this->logger->error('Failed to extract view HTML for view name: ' . $view_name . ' and display name: ' . $view_display_name);
        }
    } else {
        $this->logger->error('ViewDataService is not initialized.');
        return 'ViewDataService is not initialized.';
    }

    // Append the view HTML to the prompt
    $prompt .= "\n\nView HTML:\n" . $view_html;
...
protected function pollRunStatus($thread_id, $run_id, $api_key) {
    $client = new Client();
    $maxAttempts = 30; // Increase the maximum number of attempts
    $attempt = 0;
    $sleepTime = 5; // Increase the sleep time between attempts (in seconds)

    while ($attempt < $maxAttempts) {
        try {
            $this->logger->notice('Polling attempt: ' . ($attempt + 1));
            ...
            $this->logger->notice('Poll response data: ' . json_encode($responseData));
